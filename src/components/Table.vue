...
const reRender = (add = false) => {
    document.getElementById('close-btn').click()
    props.forceRender();
    add ? props.setAlertData('success', 'block', 'Your movie has been added to the list!', true) : props.setAlertData('success', 'block', 'Your movie has been edited.', true)
...
<template>
    <Modal :data="modalVar.modalData" :editMode='modalVar.editMode' :deleteMode="modalVar.deleteMode"
        ...
        <template v-slot:header>
            <h2 v-if="modalVar.editMode" class="modal-title fs-5" id="mainModalLabel">Edit movie</h2>
            <h2 v-else-if="modalVar.deleteMode" class="modal-title fs-5" id="mainModalLabel">Delete movie</h2>
        ...
        <template v-slot:body>
            <EditMovieForm :key="modalVar.editData?.id" v-if="modalVar.editMode" :data="modalVar.editData"
                @formSubmitted="reRender()" :setAlertData="props.setAlertData" />
            <p class="delete__msg" v-else-if="modalVar.deleteMode">Are you sure you want to delete movie: <br />
                {{ modalVar.modalData.title.toUpperCase() }}? </p>
            <AddMovieForm v-else @formSubmitted="reRender(add = true)" :setAlertData="props.setAlertData" />
        </template>
    </Modal>
                ...
                <tr v-for="(item, idx) of movies" :key="item.id">
                    ...
                    <td data-label="Rate">
                        <star-rating :read-only="true" :increment="0.5" :max-rating="1" inactive-color="#e4c000"
                            :star-size="20" :show-rating="false" :inline="true" />
                        <span class="rate__num"> {{ item.rate === 10 ? item.rate :
                                item.rate.toFixed(1)
                        }} <span class="rate__limit">/ 10</span></span>
                    </td>
                    <td data-label="Action" class="justify-content-around"> <button type="button"
                            class="btn text-secondary" data-bs-toggle="modal" data-bs-target="#mainModal"
